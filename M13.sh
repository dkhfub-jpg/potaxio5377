...
####################################
banner() {
    printf "%35s${GREEN}Script lite${RESET}\n"
}
...
checking_screen() {
    ...
    echo -e "${RESET}"
    echo
    echo -e "${GREEN}Espere unos segundos...${RESET}"
}
...
connect_auto() {
    ...
    for SERVER in "${SERVERS[@]}"; do
        ...
        clear
        echo -e "${CYAN}[*] Probando servidor para testear:${RESET} $SERVER"
        separator
        ...
        for i in {1..4}; do
            if grep -q "Connection confirmed" "$LOG_FILE"; then
                clear
                echo -e "${GREEN}${BOLD}DNS funcionando ✅${RESET}"
                echo -e "${GREEN}DNS activo:${RESET} ${YELLOW}$SERVER${RESET}"
                separator
                echo -e "${GRAY}Copie el dns marcado en amarillo, luego desactive los datos móviles/wifi e proceda a salir del termux${RESET}"
                wait $PID
                return
            fi
            sleep 1
